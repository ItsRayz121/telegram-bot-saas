      ...
      ? api.get(`/api/telegram-groups/${groupId}/mod-log`, { params }).then(r => ({
          data: {
            logs: (r.data.events || []).map(e => ({
              ...
              target_username: e.metadata?.target_username || '',
              target_user_id: e.metadata?.target_user_id || e.metadata?.telegram_user_id || '',
...
export const hub = {
  ...
  createMeeting:        (data)       => api.post('/api/hub/meetings', data),
  updateMeeting:        (id, data)   => api.patch(`/api/hub/meetings/${id}`, data),
  dismissMeeting:       (id)         => api.patch(`/api/hub/meetings/${id}/dismiss`),
  getAutomations:       (botId)      => api.get(`/api/hub/bots/${botId || 'official'}/automations`),
  updateAutomations:    (data, botId) => api.patch(`/api/hub/bots/${botId || 'official'}/automations`, data),

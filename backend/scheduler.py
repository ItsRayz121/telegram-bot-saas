...
def _send_telegram_dm(telegram_user_id: str, text: str) -> bool:
    ...
    Anti-ban (BINDING rule): delegates to the shared `safe_send_message`, which
    paces sends under Telegram's global rate cap and honors a 429 flood-wait, so
    bulk proactive loops (briefings, reminders, health alerts) can never hammer
    the API into a ban. Only ever called with users who have started the bot.
    """
    import os

# REVERT â€” how to undo anything we ship
...
read code or think â€” you should be able to open this file, find the change, and run one
line.
...
Set these in **Railway â†’ service â†’ Variables**. The service restarts and picks them up.
...
# Change log â€” newest first

---

## `<pending>` â€” AI KB: professional no-answer handling + official-bot auto-reply
**Date:** 2026-07-17 Â· **Risk:** medium Â· **Touches:** bot hot path (`on_message`, both lineages)
...
1. **The AI no longer says "I don't know" in the group.** The prompt now makes the model
   ...
   "I've passed your question to the admins â€” you'll get an answer shortly" ack in the group
...
2. **KB answers are no longer rejected by the cosine-similarity gate.** The old
   ...
   (~0.4â€“0.6), so correct PDF answers were being thrown away â€” this is why "the AI can't
...
3. **The official (shared) bot now actually has AI Auto-Reply.** The
   ...
   code read it â€” only `/ask` worked. Implemented in `official_bot.py` (mention-only by
...
- âœ… Restores the old behaviour (public "I don't know" replies, 0.65 gate, no official auto-reply).
- âš ï¸ Escalation DMs already sent to admins are not unsent (they're just messages).
- âš ï¸ Auto-learned Q&A rows (`AutoResponse` with `use_as_ai_knowledge`) created by admin
  replies stay â€” they're data the admin authored, keep them.

### Kill switch (if any)
`OFFICIAL_KB_AUTO_REPLY=0` â€” instantly disables the new official-bot auto-reply globally
...
- Auto-reply is **default OFF** and **mention-only by default** â€” no group gets new AI
...
- A real answer starting with the words "No answerâ€¦" is not misclassified as the sentinel
  (underscore-token match only; unit-smoke-tested).

---

## `d098eec` â€” fix paid custom-bot profile description crash
**Date:** 2026-07-13 Â· **Risk:** low Â· **Touches:** custom-bot startup (cosmetic profile text)
...
description set, AND â€” because the crash skipped both calls â€” a bot upgraded from free kept its
...
## `9af3685` â€” retention: make the dry-run preview visible on every deploy
**Date:** 2026-07-13 Â· **Risk:** low Â· **Touches:** scheduling only (no deletion behaviour change)
...
hourly (in-memory counter, read-only). **Real deletion is unchanged** â€” still gated to once per
...
## `957dedb` â€” Guildizer: retention sweep + XP roll-up
**Date:** 2026-07-13 Â· **Risk:** medium Â· **Touches:** data deletion (Guildizer only)

### What changed
The same sweep as `26f201e`, ported to Guildizer (`discord-board/backend/retention.py`).
Logic **copied, not imported** â€” it touches only Guildizer's engine and models. Runs as a
...
### To revert
```bash
# INSTANT, no deploy â€” Railway â†’ Guildizer bot service â†’ Variables:
#   RETENTION_ENABLED=0
...
### What revert restores, and what it does NOT
- âœ… Removes the sweep. No further rows are deleted.
- âš ï¸ Does NOT restore already-deleted rows â€” but XP is preserved in `Member.xp` (never
  touched) and archived in `xp_monthly`. **No member's XP or level changes.**
- âš ï¸ Leaves the `xp_monthly` table in place. Harmless; it holds your archived history.
  Do not drop it.

### Two traps found while building this â€” do not undo these
- **`guild_events` is NOT a log.** Despite the name it's the Discord *scheduled events*
  feature (`start_at`/`end_at`) â€” real user content. It is deliberately **excluded** from
...
- **`member_stats.xp_by_user()` sums the WHOLE ledger when `since=None`.** That would
  ...
  instead). **That guard is now load-bearing â€” do not remove it.**

---

## `26f201e` â€” cost audit: retention, AI limits, per-message query storm, TTL leaks
**Date:** 2026-07-13 Â· **Risk:** medium Â· **Touches:** money, data deletion, bot hot path

### What changed
1. **AI token limits** â€” Pro was advertised at 500k/day but *enforced* at 200k. Now 200k
   in all three places. Platform spend cap $50/day â†’ $5/day.
2. **`backend/retention.py` (new)** â€” daily sweep. Rolls expiring `xp_events` into a new
   `xp_monthly` table, then prunes it plus six other append-only tables.
3. **`official_bot.on_message`** â€” was opening 8 Flask app contexts and re-querying the
   same group row 4+ times per message. Now reads settings once and gates on them.
4. **`backend/utils/ttl_map.py` (new)** â€” three module dicts leaked one entry per user
   forever; swapped for a self-pruning `TTLMap`.

### To revert
```bash
# FIRST, if the problem is data deletion â€” this is instant, do it before anything else:
#   Railway â†’ web service â†’ Variables â†’ RETENTION_ENABLED=0
...
### What revert restores, and what it does NOT
- âœ… Restores Pro to advertising 500k/day (and back to enforcing 200k â€” i.e. **it puts the
  customer-facing bug back**). Prefer changing the numbers over reverting this part.
- âœ… Restores the $50/day AI spend cap.
- âœ… Removes the retention sweep. **No further rows are deleted.**
- âœ… Restores the old per-message query behaviour and the plain dicts.
- âš ï¸ **Does NOT bring back rows already deleted.** If the sweep already ran with
  `RETENTION_DRY_RUN=0`, those `xp_events` rows are gone â€” but their **XP is preserved**
...
- âš ï¸ The `xp_monthly` and `xp_events` tables are left in place by a revert. Harmless â€”
  an unused table costs nothing. Do not drop them; they hold your archived history.

### Partial reverts (safer than the whole thing)
| To undo justâ€¦ | Do this |
|---|---|
| the data deletion | `RETENTION_ENABLED=0` (instant, no deploy) |
| the AI limit change | edit `AI_TOKEN_LIMITS`, `PLANS['pro']`, `ai_config.AI_DEFAULTS`, `Pricing.js` â€” **all four, or they drift apart again** |
...
- The delete feeds the archive **in one transaction** â€” a crash rolls back both, so a
...
- Deletes are **batched at 5,000**. A single large `DELETE` would lock the table and stall
  the bots â€” that is the real danger here, and it is handled.
...
## `f496071` + `e972d53` + `79647f3` â€” Railway cost leak / celery-worker idled
**Date:** 2026-07-10 â†’ 2026-07-12 Â· **Risk:** medium Â· **Touches:** infrastructure

### What changed
`celery-worker` was idled. Every Celery task was calling `create_app()`, which started a
duplicate set of bots and a second scheduler loop â€” up to 200Ã— per worker child. All 19
jobs moved into the web service's in-process scheduler (`_scheduler_loop` in
`backend/app.py`). Bill went $34 â†’ ~$11.
...
git push origin main
```
**Order matters** â€” revert `f496071` first.
...
- âš ï¸ The `scheduled_job_runs` table is what stops a redeploy from re-firing daily jobs.
  If you revert and daily emails go out twice, that table is why.

---

## Template â€” copy this for the next change

```markdown
## `<sha>` â€” <one-line summary>
**Date:** Â· **Risk:** low/medium/high Â· **Touches:** money / data deletion / hot path / plan limits

### What changed
<plain English, 2â€“4 bullets>
...
### What revert restores, and what it does NOT
- âœ… â€¦
- âš ï¸ â€¦ (especially: anything deleted, sent, or charged is NOT undone by a revert)
